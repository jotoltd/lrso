#!/usr/bin/env python3
"""Patch venue names and addresses using direct REST API calls."""
import requests
...
PATCHES = [
    ...
    ("420e1842-f122-4d79-a005-f661a8362c08", "Murrayfield Primary",              "Nacton Road, Ipswich, IP3 9JL"),
...
for vid, name, address in PATCHES:
    r = requests.patch(
        f"{SUPABASE_URL}/rest/v1/venues?id=eq.{vid}",
        json={"name": name, "address": address},
        headers=H,
    )
    icon = "✅" if r.status_code in (200, 204) else f"❌ {r.status_code}: {r.text}"
    print(f"{icon}  {name}")
